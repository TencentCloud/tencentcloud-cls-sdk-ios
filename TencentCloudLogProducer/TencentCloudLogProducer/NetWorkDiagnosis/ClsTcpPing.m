...
- (void)run {
    ...
    if (addr.sin_addr.s_addr == INADDR_NONE) {
        ...
        if (host == NULL || host->h_addr == NULL) {
            ...
            if (_complete != nil && !_stopped) {
                ...
                [_sender clsReport:[self buildResult:-1006 ip:nil domain:_host durations:nil loss:0 port:_port count:0 totalTime:0].description method:@"TCPPing" domain:_host customFiled:_tcpPingExt];
            }
            return;
        }
    ...
    if (!_stopped){
        [_sender clsReport:[self buildResult:code ip:ip domain:_host durations:intervals loss:loss port:_port count:index totalTime:[[NSDate date] timeIntervalSinceDate:begin] * 1000].description method:@"tcpPing" domain:_host customFiled:_tcpPingExt];
    }
}
