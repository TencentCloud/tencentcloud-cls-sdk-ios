...
- (void)run{
    ...
    if (addr.sin_addr.s_addr == INADDR_NONE) { //无效的地址
        ...
        if (host == NULL || host->h_addr == NULL) {
            ...
            if (_complete != nil && !_stopped) {
                ...
                [_sender clsReport:[[CLSPingResult alloc] init:-1006 err_msg:@"host illegal" ip:nil domain:_host size:_size max:0 min:0 avg:0 loss:0 count:0 totalTime:0 stddev:0].description method:@"ping" domain:_host customFiled:_pingExt];
            }
            return;
        }
    ...
    //发送数据到cls
    if (!_stopped){
        [_sender clsReport:[self buildResult:r ip:[NSString stringWithUTF8String:inet_ntoa(addr.sin_addr)]
                                           domain:_host
                                        durations:durations
                                            count:index
                                             loss:loss
