 ...
 * CURLSCLSET_UNKNOWN_BACKEND and set the "avail" pointer to the
 * NULL-terminated list of available backends.
 *
 * Upon success, the function returns CURLSCLSET_OK.
 *
 * If the specified SSL backend is not available, the function returns
 * CURLSCLSET_UNKNOWN_BACKEND and sets the "avail" pointer to a NULL-terminated
 * list of available SSL backends.
 *
 * The SSL backend can be set only once. If it has already been set, a
 * subsequent attempt to change it will result in a CURLSCLSET_TOO_LATE.
 */
...
typedef enum {
  CURLSCLSET_OK = 0,
  CURLSCLSET_UNKNOWN_BACKEND,
  CURLSCLSET_TOO_LATE,
  CURLSCLSET_NO_BACKENDS /* libcurl was built without any SSL support */
} CURLsslset;
