...
// 旧实现为固定数组 + 满了丢最老事件，容易系统性扭曲统计（loss/latency/stddev/IP
// 选择）。
...
// TCP 探测：send_tcp_syn 内部会额外 sleep（确保 SYN
// 发出），因此超时估算使用更保守的 gap
...
// 前向声明
static socket_t create_socket(const char *protocol, int is_ipv6,
                              unsigned int interface_index,
                              int *out_bind_failed, int *out_errno);
static int udp_probe_decode_from_dst_port(uint16_t dst_port, uint16_t base,
                                          int *ttl_out, int *probe_index_out);
static int parse_icmp_embedded_udp_ports(const char *buffer, size_t buffer_len,
                                         int is_ipv6, uint8_t *icmp_type_out,
                                         uint8_t *icmp_code_out,
                                         uint16_t *orig_src_port_out,
                                         uint16_t *orig_dst_port_out);
...
// 辅助函数：获取单调时间戳（毫秒）
// - 用于 RTT/超时判断，避免系统时间跳变导致 RTT<=0 被误判为丢包或提前触发
// deadline
static uint64_t get_monotonic_timestamp_ms(void) {
  static mach_timebase_info_data_t timebase_info;
  if (timebase_info.denom == 0) {
    (void)mach_timebase_info(&timebase_info);
  }
  const uint64_t t = mach_absolute_time();
  const __uint128_t ns = (__uint128_t)t * (__uint128_t)timebase_info.numer /
                         (__uint128_t)timebase_info.denom;
...
static uint64_t mtr_compute_parallel_session_timeout_ms(int max_ttl, int times,
                                                        int timeout_ms,
                                                        uint32_t probe_gap_us) {
  if (max_ttl < 1)
    max_ttl = 1;
  if (times < 1)
    times = 1;
  ...
  uint64_t send_phase_ms =
      ((uint64_t)max_ttl * (uint64_t)times * (uint64_t)probe_gap_us + 999) /
      1000;
...
static const char *mtr_error_name(cls_mtr_detector_error_code code) {
  switch (code) {
  case cls_mtr_detector_error_success:
    return "success";
  case cls_mtr_detector_error_invalid_target:
    return "invalid_target";
  case cls_mtr_detector_error_network_unreachable:
    return "network_unreachable";
  case cls_mtr_detector_error_timeout:
    return "timeout";
  case cls_mtr_detector_error_permission_denied:
    return "permission_denied";
  case cls_mtr_detector_error_socket_create_error:
    return "socket_create_error";
  case cls_mtr_detector_error_resolve_error:
    return "resolve_error";
  case cls_mtr_detector_error_net_binding_failed:
    return "net_binding_failed";
  case cls_mtr_detector_error_invalid_param:
    return "invalid_param";
  case cls_mtr_detector_error_setsockopt_failed:
    return "setsockopt_failed";
  case cls_mtr_detector_error_send_failed:
    return "send_failed";
  case cls_mtr_detector_error_recv_failed:
    return "recv_failed";
  case cls_mtr_detector_error_connect_failed:
    return "connect_failed";
  case cls_mtr_detector_error_host_unreachable:
    return "host_unreachable";
  case cls_mtr_detector_error_resource_exhausted:
    return "resource_exhausted";
  case cls_mtr_detector_error_address_not_available:
    return "address_not_available";
  case cls_mtr_detector_error_address_in_use:
    return "address_in_use";
  case cls_mtr_detector_error_unknown_error:
  default:
    return "unknown_error";
  }
}

static cls_mtr_detector_error_code mtr_classify_syscall_failure(const char *op,
                                                                int e) {
  cls_mtr_detector_error_code base = mtr_error_code_from_errno(e);
  if (!op)
    return base;
  // 对“操作类型”做更细分（比单纯 errno 更可解释）
  if (strcmp(op, "setsockopt") == 0)
    return (base == cls_mtr_detector_error_unknown_error)
               ? cls_mtr_detector_error_setsockopt_failed
               : base;
  if (strcmp(op, "sendto") == 0 || strcmp(op, "send") == 0)
    return (base == cls_mtr_detector_error_unknown_error)
               ? cls_mtr_detector_error_send_failed
               : base;
  if (strcmp(op, "recvfrom") == 0 || strcmp(op, "recvmsg") == 0 ||
      strcmp(op, "select") == 0 || strcmp(op, "recv") == 0)
    return (base == cls_mtr_detector_error_unknown_error)
               ? cls_mtr_detector_error_recv_failed
               : base;
  if (strcmp(op, "connect") == 0)
    return (base == cls_mtr_detector_error_unknown_error)
               ? cls_mtr_detector_error_connect_failed
               : base;
  return base;
}

static void mtr_record_fatal_error(cls_mtr_path_result *path, const char *op,
                                   int e) {
  if (!path)
    return;
  path->last_errno = e;
...
static int resolve_hostname(const char *hostname, int prefer, char *ip_buffer) {
  ...
  if (selected->ai_family == AF_INET) {
    ...
    ntop_result = (inet_ntop(AF_INET, &addr_in->sin_addr, ip_buffer,
                             INET6_ADDRSTRLEN) != NULL)
                      ? 0
                      : -1;
  } else if (selected->ai_family == AF_INET6) {
    struct sockaddr_in6 *addr_in6 = (struct sockaddr_in6 *)selected->ai_addr;
    ntop_result = (inet_ntop(AF_INET6, &addr_in6->sin6_addr, ip_buffer,
                             INET6_ADDRSTRLEN) != NULL)
                      ? 0
                      : -1;
  } else {
...
// 初始化路径结果
static void init_path_result(cls_mtr_path_result *path, const char *target,
  ...
  safe_strncpy(path->protocol, protocol ? protocol : "",
               sizeof(path->protocol));
...
static int expand_path_results(cls_mtr_path_result *path, size_t new_capacity) {
  if (!path || new_capacity <= path->results_capacity)
    return 0;

  cls_mtr_hop_result *new_results =
      realloc(path->results, new_capacity * sizeof(cls_mtr_hop_result));
  if (!new_results)
    return -1;
...
// 确保路径结果数组有足够的容量
static int ensure_path_results_capacity(cls_mtr_path_result *path,
  ...
  if (required_count > path->results_capacity) {
    size_t new_capacity =
        path->results_capacity == 0 ? 32 : path->results_capacity * 2;
...
static int receive_icmp_response(socket_t sock, int timeout_ms,
                                 const char *target_ip, int ttl, int is_ipv6,
                                 double *rtt_out, char *src_ip_out,
                                 uint64_t send_time, uint16_t expected_id);

// 前置声明：解析 ICMP 响应
static int parse_icmp_response(const char *buffer, size_t buffer_len,
                               int is_ipv6, int *original_ttl,
                               int *original_seq, int *is_echo_reply,
...
typedef struct {
  ...
  // TCP 专属：用于更保守的“到达目标”判定
  int tcp_refused_count;   // ECONNREFUSED 次数（弱证据）
  int tcp_connected_count; // connect 成功次数（强证据）
} mtr_hop_state;
...
static void fill_hop_result_from_state(cls_mtr_hop_result *hop_result,
                                       const mtr_hop_state *hop_state, int ttl,
                                       int times) {
  if (!hop_result)
    return;
  init_hop_result(hop_result, ttl);
  if (!hop_state || times <= 0)
    return;
  ...
  // 计算 RTT 统计（只统计 rtt>0 的样本，避免 rtts 与 recv_count
  // 不一致导致除零/偏差）
  double sum = 0.0;
  double min_rtt = 0.0;
  double max_rtt = 0.0;
...
static int mtr_check_consecutive_timeouts_and_stop(
    mtr_hop_state *hops, int max_ttl, int times, int timeout_ms,
    uint64_t now_ms, int limit_ttl, cls_mtr_path_result *path,
    const char *log_tag) {
  if (!hops || max_ttl < 1 || times < 1 || timeout_ms < 1 || limit_ttl < 1)
    return 0;
  if (limit_ttl > max_ttl)
    limit_ttl = max_ttl;
...
typedef struct {
  ...
  // session 级 token：不依赖 ICMP ID（可能被内核改写），而是对 sequence 做 XOR
  // 编码 send: seq_wire = ((ttl<<8)|probe_index) ^ seq_xor_token recv: decoded
  // = seq_wire ^ seq_xor_token -> (ttl, probe_index)
...
typedef struct {
  ...
  int is_target_refused; // 是否为 ECONNREFUSED（弱证据：可能为目标或中间盒子）
} mtr_tcp_event;
...
typedef struct {
  ...
  int times;
  int timeout_ms;
  uint16_t tcp_dst_port; // TCP 探测目的端口
...
static int send_icmp_echo(socket_t icmp_socket, const char *target_ip, int ttl,
                          int sequence, uint16_t icmp_identifier,
                          uint16_t seq_xor_token, uint64_t *send_time,
                          int *out_errno, const char **out_failed_op) {
  if (!target_ip || icmp_socket < 0 || ttl < 1 || ttl > 255)
    return -1;
  if (out_errno)
    *out_errno = 0;
  if (out_failed_op)
  ...
  snprintf(icmp_packet.data, sizeof(icmp_packet.data), "MTR-%d-%d", ttl,
           sequence);
  ...
  if (!is_ipv6) {
    ...
    icmp_packet.checksum =
        calculate_icmp_checksum(&icmp_packet, sizeof(icmp_packet));
  }
  ...
  if (sent < 0 || sent != (ssize_t)sizeof(icmp_packet)) {
    if (out_errno)
      *out_errno = errno;
    if (out_failed_op)
      *out_failed_op = "sendto";
    return -1;
  }

  if (send_time) {
    *send_time = get_monotonic_timestamp_ms();
  }
...
static int mtr_compute_event_queue_capacity(int max_ttl, int times) {
  ...
  if (cap < (uint64_t)MTR_EVENT_QUEUE_MIN_CAPACITY)
    cap = (uint64_t)MTR_EVENT_QUEUE_MIN_CAPACITY;
  if (cap > (uint64_t)MTR_EVENT_QUEUE_MAX_CAPACITY)
    cap = (uint64_t)MTR_EVENT_QUEUE_MAX_CAPACITY;
  return (int)cap;
}
...
static int event_queue_grow_locked(mtr_event_queue *queue, int min_capacity) {
  if (!queue)
    return -1;
  if (min_capacity < MTR_EVENT_QUEUE_MIN_CAPACITY)
    min_capacity = MTR_EVENT_QUEUE_MIN_CAPACITY;
  int new_cap =
      queue->capacity > 0 ? queue->capacity : MTR_EVENT_QUEUE_MIN_CAPACITY;
  while (new_cap < min_capacity && new_cap < MTR_EVENT_QUEUE_MAX_CAPACITY) {
    new_cap *= 2;
  }
  if (new_cap > MTR_EVENT_QUEUE_MAX_CAPACITY)
    new_cap = MTR_EVENT_QUEUE_MAX_CAPACITY;
  if (new_cap < min_capacity)
    return -1;

  mtr_icmp_event *new_events =
      (mtr_icmp_event *)calloc((size_t)new_cap, sizeof(mtr_icmp_event));
  if (!new_events)
    return -1;
...
static int event_queue_init(mtr_event_queue *queue, int capacity) {
  ...
  if (capacity < MTR_EVENT_QUEUE_MIN_CAPACITY)
    capacity = MTR_EVENT_QUEUE_MIN_CAPACITY;
  if (capacity > MTR_EVENT_QUEUE_MAX_CAPACITY)
    capacity = MTR_EVENT_QUEUE_MAX_CAPACITY;
  queue->capacity = capacity;
  queue->events =
      (mtr_icmp_event *)calloc((size_t)queue->capacity, sizeof(mtr_icmp_event));
  if (!queue->events)
    return -1;
...
static int event_queue_push(mtr_event_queue *queue,
                            const mtr_icmp_event *event) {
  if (!queue || !event)
    return -1;
...
// UDP 事件队列操作函数（动态环形队列）
static int udp_event_queue_grow_locked(mtr_udp_event_queue *queue,
                                       int min_capacity) {
  if (!queue)
    return -1;
  if (min_capacity < MTR_EVENT_QUEUE_MIN_CAPACITY)
  ...
  mtr_udp_event *new_events =
      (mtr_udp_event *)calloc((size_t)new_cap, sizeof(mtr_udp_event));
  if (!new_events)
    return -1;
  for (int i = 0; i < queue->count; i++) {
    int idx = (queue->head + i) % queue->capacity;
...
static int udp_event_queue_init(mtr_udp_event_queue *queue, int capacity) {
  ...
  queue->capacity = capacity;
  queue->events =
      (mtr_udp_event *)calloc((size_t)queue->capacity, sizeof(mtr_udp_event));
  if (!queue->events)
    return -1;
  pthread_mutex_init(&queue->mutex, NULL);
  return 0;
...
static int udp_event_queue_push(mtr_udp_event_queue *queue,
                                const mtr_udp_event *event) {
  if (!queue || !event)
    return -1;
  pthread_mutex_lock(&queue->mutex);
  if (!queue->events || queue->capacity <= 0) {
...
static int udp_event_queue_try_pop(mtr_udp_event_queue *queue,
                                   mtr_udp_event *event) {
  if (!queue || !event)
    return -1;
  pthread_mutex_lock(&queue->mutex);
...
// TCP 事件队列操作函数（动态环形队列）
static int tcp_event_queue_grow_locked(mtr_tcp_event_queue *queue,
                                       int min_capacity) {
  if (!queue)
    return -1;
  if (min_capacity < MTR_EVENT_QUEUE_MIN_CAPACITY)
  ...
  mtr_tcp_event *new_events =
      (mtr_tcp_event *)calloc((size_t)new_cap, sizeof(mtr_tcp_event));
  if (!new_events)
    return -1;
  for (int i = 0; i < queue->count; i++) {
    int idx = (queue->head + i) % queue->capacity;
...
static int tcp_event_queue_init(mtr_tcp_event_queue *queue, int capacity) {
  ...
  queue->capacity = capacity;
  queue->events =
      (mtr_tcp_event *)calloc((size_t)queue->capacity, sizeof(mtr_tcp_event));
  if (!queue->events)
    return -1;
  pthread_mutex_init(&queue->mutex, NULL);
  return 0;
...
static int tcp_event_queue_push(mtr_tcp_event_queue *queue,
                                const mtr_tcp_event *event) {
  if (!queue || !event)
    return -1;
  pthread_mutex_lock(&queue->mutex);
  if (!queue->events || queue->capacity <= 0) {
...
static int tcp_event_queue_try_pop(mtr_tcp_event_queue *queue,
                                   mtr_tcp_event *event) {
  if (!queue || !event)
    return -1;
  pthread_mutex_lock(&queue->mutex);
...
static void *icmp_parallel_worker_thread(void *arg) {
  ...
  while (1) {
    ...
    uint64_t recv_time = get_monotonic_timestamp_ms();
    ...
    if (parse_icmp_response(buffer, (size_t)bytes_received, is_ipv6,
                            &original_ttl, &original_seq, &is_echo_reply,
                            &icmp_id_value) != 0) {
      continue;
    }
...
typedef struct {
  ...
  mtr_udp_event_queue *event_queue;
  int is_ipv6;
  uint16_t dst_port_base; // UDP 探测目的端口基址：dst_port = base + (ttl<<8) +
                          // probe_index
  ...
  while (1) {
    ...
    if (icmp_sock >= 0 && FD_ISSET(icmp_sock, &read_fds)) {
      ...
      ssize_t bytes_received =
          recvfrom(icmp_sock, buffer, sizeof(buffer), 0,
                   (struct sockaddr *)&from_addr, &from_len);
      if (bytes_received > 0) {
        // 提取源 IP
        if (from_addr.ss_family == AF_INET6) {
        ...
        if (ip_version == 4 && bytes_received >= 20) {
          ...
          if (ip_hdr_len >= 20 && ip_hdr_len <= bytes_received &&
              bytes_received >= ip_hdr_len + 8) {
        ...
        // 优先使用“内嵌 UDP 端口解码”的方式精确匹配 (ttl, probe_index)
        {
          ...
          uint16_t orig_sp = 0;
          uint16_t orig_dp = 0;
          if (parse_icmp_embedded_udp_ports(buffer, (size_t)bytes_received,
                                            is_ipv6, &icmp_type2, &icmp_code2,
                                            &orig_sp, &orig_dp) == 0) {
            int dt = 0;
            int dp = -1;
            if (udp_probe_decode_from_dst_port(orig_dp, dst_port_base, &dt,
                                               &dp) == 0) {
              ttl = dt;
              probe_index = dp;
            }
        ...
        if (ttl <= 0) {
          int parse_ret = parse_icmp_response(
              buffer, (size_t)bytes_received, is_ipv6, &original_ttl,
              &original_seq, &is_echo_reply, &icmp_id_value);
          if (parse_ret == 0) {
            ttl = original_ttl;
            if (ttl == 0)
              ttl = 1;
    ...
    if (FD_ISSET(udp_sock, &read_fds) || FD_ISSET(udp_sock, &except_fds)) {
      socklen_t from_len = sizeof(from_addr);
      ssize_t bytes_received =
          recvfrom(udp_sock, buffer, sizeof(buffer), MSG_DONTWAIT,
                   (struct sockaddr *)&from_addr, &from_len);
      if (bytes_received > 0) {
        // 提取源 IP
        if (from_addr.ss_family == AF_INET6) {
...
static int udp_parallel_worker_start(udp_parallel_worker_ctx *ctx,
                                     socket_t udp_sock, socket_t icmp_sock,
                                     mtr_udp_event_queue *queue, int is_ipv6,
                                     uint16_t dst_port_base) {
  if (!ctx || udp_sock < 0 || !queue)
    return -1;
...
typedef struct {
  ...
  uint16_t src_port_base; // TCP 探测源端口基址：src_port = base + (ttl<<8) +
                          // probe_index
  uint16_t tcp_dst_port;  // TCP 探测目的端口
...
static int tcp_probe_decode_from_src_port(uint16_t src_port, uint16_t base,
                                          int *ttl_out, int *probe_index_out) {
  if (!ttl_out || !probe_index_out)
    return -1;
  if (src_port < base)
    return -1;
...
static int mtr_ipv6_locate_transport_header(const uint8_t *ipv6_pkt,
                                            size_t ipv6_len, uint8_t *proto_out,
                                            const uint8_t **l4_out,
                                            size_t *l4_len_out) {
  if (!ipv6_pkt || ipv6_len < 40 || !proto_out || !l4_out || !l4_len_out)
    return -1;
...
static int parse_icmp_embedded_tcp_ports(const char *buffer, size_t buffer_len,
                                         int is_ipv6, uint8_t *icmp_type_out,
                                         uint8_t *icmp_code_out,
                                         uint16_t *orig_src_port_out,
                                         uint16_t *orig_dst_port_out) {
  if (!buffer || buffer_len < 8 || !orig_src_port_out || !orig_dst_port_out)
    return -1;
  ...
  if (ip_version == 4 && buffer_len >= sizeof(struct ip)) {
    const struct ip *ip_hdr = (const struct ip *)buffer;
    size_t ip_hdr_len = ip_hdr->ip_hl * 4;
    if (ip_hdr_len >= 20 && ip_hdr_len <= buffer_len &&
        buffer_len >= ip_hdr_len + 8) {
      icmp_base = buffer + ip_hdr_len;
  ...
  if (icmp_code_out)
    *icmp_code_out = icmp_code;

  // Echo Reply 不属于 TCP 探测响应
  if ((!is_ipv6 && icmp_type == 0) || (is_ipv6 && icmp_type == 129))
    return -1;
  ...
  if (inner_ver == 4) {
    ...
    size_t l4plen = 0;
    if (mtr_ipv6_locate_transport_header((const uint8_t *)inner, inner_len,
                                         &proto, &l4p, &l4plen) != 0)
      return -1;
    l4 = (const char *)l4p;
    l4_len = l4plen;
...
static socket_t create_tcp_probe_socket(int is_ipv6,
                                        unsigned int interface_index,
                                        uint16_t bind_port) {
  ...
  if (interface_index > 0) {
    if (!is_ipv6) {
      setsockopt(sock, IPPROTO_IP, IP_BOUND_IF, &interface_index,
                 sizeof(interface_index));
    } else {
#ifdef IPV6_BOUND_IF
      setsockopt(sock, IPPROTO_IPV6, IPV6_BOUND_IF, &interface_index,
                 sizeof(interface_index));
#endif
    }
  }
  ...
  while (1) {
    ...
    for (int i = 0; i < tcp_socket_count; i++) {
      if (tcp_sockets[i] >= 0) {
        ...
        if (tcp_sockets[i] > max_fd)
          max_fd = tcp_sockets[i];
      }
    }
    pthread_mutex_unlock(&ctx->mutex);
    ...
    // 优先检查 ICMP socket（ICMP 错误消息：Time Exceeded、Destination
    // Unreachable 等）
    if (icmp_sock >= 0 && FD_ISSET(icmp_sock, &read_fds)) {
      socklen_t from_len = sizeof(from_addr);
      ssize_t bytes_received =
      ...
      if (bytes_received > 0) {
        ...
        int p_ret = parse_icmp_embedded_tcp_ports(
            buffer, (size_t)bytes_received, is_ipv6, &icmp_type, &icmp_code,
            &orig_src_port, &orig_dst_port);
        if (p_ret == 0) {
          int decoded_ttl = 0;
          int decoded_probe = -1;
          if (tcp_probe_decode_from_src_port(orig_src_port, port_base,
                                             &decoded_ttl,
                                             &decoded_probe) == 0) {
            ttl = decoded_ttl;
    ...
    for (int i = 0; i < tcp_socket_count; i++) {
      ...
      if (FD_ISSET(sock, &write_fds) || FD_ISSET(sock, &except_fds)) {
        ...
        if (getsockopt(sock, SOL_SOCKET, SO_ERROR, &so_error, &error_len) ==
            0) {
          if (so_error == 0 || so_error == ECONNREFUSED) {
            ...
            int decoded_probe = -1;
            if (local_port != 0 &&
                tcp_probe_decode_from_src_port(
                    local_port, port_base, &decoded_ttl, &decoded_probe) == 0) {
              ttl = decoded_ttl;
              probe_index = decoded_probe;
              // 更保守的判定：
              // - connect 成功(so_error==0) 视为“强证据到达目标”
              // - ECONNREFUSED
              // 仅为“弱证据”，不直接判定到达目标（可能来自中间盒子）
              is_target_reply = (so_error == 0) ? 1 : 0;
              ...
              event.recv_time = recv_time;
              event.is_target_reply = is_target_reply;
              event.is_target_refused = (so_error == ECONNREFUSED) ? 1 : 0;
...
static int tcp_parallel_worker_start(tcp_parallel_worker_ctx *ctx,
                                     socket_t *tcp_sockets,
                                     int tcp_socket_count, socket_t icmp_sock,
                                     mtr_tcp_event_queue *queue, int is_ipv6,
                                     const char *target_ip,
                                     uint16_t src_port_base,
                                     uint16_t tcp_dst_port) {
  if (!ctx || !tcp_sockets || tcp_socket_count <= 0 || !queue || !target_ip)
    return -1;
...
static int send_udp_packet(socket_t udp_socket, const char *target_ip, int ttl,
                           int sequence, uint16_t dst_port_base,
                           uint16_t *src_port, uint64_t *send_time,
                           int *out_errno, const char **out_failed_op) {
  if (!target_ip || udp_socket < 0 || ttl < 1 || ttl > 255)
    return -1;
  if (out_errno)
    *out_errno = 0;
  if (out_failed_op)
  ...
  int ttl_value = ttl;
  if (is_ipv6) {
    if (setsockopt(udp_socket, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &ttl_value,
                   sizeof(ttl_value)) < 0) {
      if (out_errno)
        *out_errno = errno;
      if (out_failed_op)
        *out_failed_op = "setsockopt";
      return -1;
    }
  } else {
    if (setsockopt(udp_socket, IPPROTO_IP, IP_TTL, &ttl_value,
                   sizeof(ttl_value)) < 0) {
      if (out_errno)
        *out_errno = errno;
      if (out_failed_op)
  ...
  if (src_port) {
    if (is_ipv6) {
      ...
      if (getsockname(udp_socket, (struct sockaddr *)&bound_addr, &addr_len) ==
          0) {
  ...
  if (is_ipv6) {
    ...
    sent = sendto(udp_socket, data, sizeof(data), 0, (struct sockaddr *)&addr6,
                  sizeof(addr6));
  } else {
    struct sockaddr_in addr4;
    memset(&addr4, 0, sizeof(addr4));
    ...
    sent = sendto(udp_socket, data, sizeof(data), 0, (struct sockaddr *)&addr4,
                  sizeof(addr4));
  }
...
static int send_tcp_syn(socket_t tcp_socket, const char *target_ip, int ttl,
                        int sequence, uint16_t tcp_dst_port, uint16_t *src_port,
                        uint64_t *send_time, int *out_errno,
                        const char **out_failed_op) {
  if (!target_ip || tcp_socket < 0 || ttl < 1 || ttl > 255)
    return -1;
  if (out_errno)
    *out_errno = 0;
  if (out_failed_op)
  ...
  int ttl_value = ttl;
  if (is_ipv6) {
    if (setsockopt(tcp_socket, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &ttl_value,
                   sizeof(ttl_value)) < 0) {
      if (out_errno)
        *out_errno = errno;
      if (out_failed_op)
    ...
    if (setsockopt(tcp_socket, IPPROTO_IP, IP_TTL, &ttl_value,
                   sizeof(ttl_value)) < 0) {
      if (out_errno)
        *out_errno = errno;
      if (out_failed_op)
  ...
  if (src_port) {
    if (is_ipv6) {
      struct sockaddr_in6 bound_addr;
      socklen_t addr_len = sizeof(bound_addr);
      if (getsockname(tcp_socket, (struct sockaddr *)&bound_addr, &addr_len) ==
          0) {
        *src_port = ntohs(bound_addr.sin6_port);
      } else {
        *src_port = 0;
  ...
  if (is_ipv6) {
    ...
    addr6.sin6_port = htons(tcp_dst_port);
    connect_result =
        connect(tcp_socket, (struct sockaddr *)&addr6, sizeof(addr6));
  } else {
    struct sockaddr_in addr4;
    memset(&addr4, 0, sizeof(addr4));
    addr4.sin_family = AF_INET;
    ...
    addr4.sin_port = htons(tcp_dst_port);
    connect_result =
        connect(tcp_socket, (struct sockaddr *)&addr4, sizeof(addr4));
  }
  ...
  if (connect_result < 0) {
    if (errno == EINPROGRESS) {
      ...
      if (errno == EPERM || errno == EACCES || errno == ENETUNREACH ||
          errno == EHOSTUNREACH || errno == EADDRNOTAVAIL || errno == ENOBUFS ||
          errno == ENOMEM) {
        if (out_errno)
          *out_errno = errno;
        if (out_failed_op)
          *out_failed_op = "connect";
        fcntl(tcp_socket, F_SETFL, flags);
        return -1;
      }
    }
...
static int parse_icmp_response(const char *buffer, size_t buffer_len,
                               ...
                               int *original_seq, int *is_echo_reply,
                               uint16_t *icmp_identifier) {
  if (!buffer || buffer_len < sizeof(struct mtr_icmphdr))
    return -1;
  ...
  if (ip_version == 4 && buffer_len >= sizeof(struct ip)) {
    ...
    size_t ip_hdr_len = ip_hdr->ip_hl * 4;
    if (ip_hdr_len >= 20 && ip_hdr_len <= buffer_len &&
        buffer_len >= ip_hdr_len + sizeof(struct mtr_icmphdr)) {
      icmp_base = buffer + ip_hdr_len;
      icmp_len = buffer_len - ip_hdr_len;
      if (original_ttl)
        *original_ttl = ip_hdr->ip_ttl; // 保存原始 TTL
    }
  } else if (ip_version == 6 && buffer_len >= 40) {
    ...
    if (buffer_len >= ip6_hdr_len + sizeof(struct mtr_icmphdr)) {
      ...
      if (original_ttl)
        *original_ttl = buffer[7]; // IPv6 Hop Limit 在偏移7
    }
  }
  ...
  uint8_t echo_reply_type =
      is_ipv6 ? 129 : 0; // ICMP6_ECHO_REPLY : ICMP_ECHOREPLY
  uint8_t time_exceeded_type =
      is_ipv6 ? 3 : 11; // ICMP6_TIME_EXCEEDED : ICMP_TIME_EXCEEDED
  uint8_t dest_unreach_type =
      is_ipv6 ? 1 : 3; // ICMP6_DST_UNREACH : ICMP_DEST_UNREACH
  ...
  if (icmp_hdr->type == echo_reply_type) {
    ...
    if (icmp_identifier)
      *icmp_identifier = id_value;
    if (original_seq)
      *original_seq = seq_value;
    if (original_ttl)
      *original_ttl = 0; // Echo Reply 无法获取原始 TTL
    return 0;
  } else if (icmp_hdr->type == time_exceeded_type ||
             icmp_hdr->type == dest_unreach_type) {
    ...
    if (ip_version == 4) {
      ...
      if (orig_ip_hdr_len < 20 || orig_ip_hdr_len > 60)
        orig_ip_hdr_len = 20;
      if (original_ttl)
        *original_ttl = orig_ip->ip_ttl;
    } else if (ip_version == 6) {
      orig_ip_hdr_len = 40;
      if (icmp_len < 8 + 40)
        return -1;
      if (original_ttl)
        *original_ttl = ((const uint8_t *)(icmp_base + 8))[7];
    } else {
      return -1;
    }
    ...
    if (icmp_len >= 8 + orig_ip_hdr_len + sizeof(struct mtr_icmphdr)) {
      const struct mtr_icmphdr *orig_icmp =
          (const struct mtr_icmphdr *)(icmp_base + 8 + orig_ip_hdr_len);
    ...
    if (icmp_len >= 8 + orig_ip_hdr_len + 8) { // UDP 头至少 8 字节
      const struct udphdr *orig_udp =
          (const struct udphdr *)(icmp_base + 8 + orig_ip_hdr_len);
      ...
      if (dst_port >= MTR_DST_PORT_BASE && dst_port < MTR_DST_PORT_BASE + 256) {
        ...
        if (extracted_ttl >= 1 && extracted_ttl <= 255) {
          if (original_ttl)
            *original_ttl = extracted_ttl;
          if (original_seq)
            *original_seq = extracted_ttl; // 使用 TTL 作为序列号
          return 0;
        }
      }
    }
    ...
    if (icmp_len >= 8 + orig_ip_hdr_len + 8) {
      // 注意：TCP 协议无法仅从目的端口提取 TTL（端口是配置的，不编码 TTL）
      // 这里仅用于识别是否为 TCP 协议，实际匹配应使用 parse_icmp_embedded_tcp_ports
      // 返回 -1 让调用者使用其他方式匹配
      return -1;
    }
  }
...
static int receive_icmp_response(socket_t sock, int timeout_ms,
                                 ...
                                 double *rtt_out, char *src_ip_out,
                                 uint64_t send_time, uint16_t expected_id) {
  if (sock < 0 || !target_ip || !rtt_out)
    return -1;

  uint64_t start_time = get_monotonic_timestamp_ms();
  uint16_t expected_id_host = (expected_id != 0) ? ntohs(expected_id) : 0;

  // 循环接收，直到超时或收到匹配的响应
  while (1) {
    // 检查是否超时
    uint64_t current_time = get_monotonic_timestamp_ms();
    ...
    uint64_t receive_time = get_monotonic_timestamp_ms();
    ...
    // 部分设备返回的 ICMP 可能只携带 IP 头中的 TTL（original_ttl）或序列号为
    // 0。 匹配策略：seq==ttl 或 ttl==original_ttl（两者其一即可）。
    if (original_seq != ttl) {
      if (original_ttl != ttl) {
...
// 接收 UDP 错误消息（从错误队列接收 ICMP Time Exceeded/Destination
// Unreachable） recv_icmp_sock: 可选的 ICMP DGRAM socket，用于接收 ICMP
// 错误/Time Exceeded
static __attribute__((unused)) int
receive_udp_error(socket_t udp_sock, socket_t recv_icmp_sock, int timeout_ms,
                  const char *target_ip, int ttl, int is_ipv6, double *rtt_out,
                  char *src_ip_out, uint64_t send_time) {
  if (udp_sock < 0 || !target_ip || !rtt_out)
    return -1;
  ...
  if (recv_icmp_sock >= 0) {
    ...
    int icmp_res =
        receive_icmp_response(recv_icmp_sock, timeout_ms, target_ip, ttl,
                              is_ipv6, &icmp_rtt, icmp_src_ip, send_time, 0);
    if (icmp_res == 0 && icmp_src_ip[0] != '\0') {
      if (rtt_out)
        *rtt_out = icmp_rtt;
      if (src_ip_out)
        safe_strncpy(src_ip_out, icmp_src_ip, INET6_ADDRSTRLEN);
      return 0;
    }
  }
  ...
  int recverr = 1;
  if (is_ipv6) {
    if (setsockopt(udp_sock, IPPROTO_IPV6, IPV6_RECVERR, &recverr,
                   sizeof(recverr)) < 0) {
    }
  } else {
    if (setsockopt(udp_sock, IPPROTO_IP, IP_RECVERR, &recverr,
                   sizeof(recverr)) < 0) {
    }
  }
  ...
  ssize_t bytes_received =
      recvfrom(udp_sock, data_buffer, sizeof(data_buffer), MSG_DONTWAIT,
               (struct sockaddr *)&from_addr, &from_len);
  if (bytes_received < 0) {
        ...
        if (cmsg->cmsg_len >= sizeof(struct sock_extended_err)) {
          ...
          if (serr && (serr->ee_origin == SO_EE_ORIGIN_ICMP ||
            ...
            if (cmsg->cmsg_len >= CMSG_LEN(sizeof(struct sockaddr_in)) &&
              ...
              if (src_ip_out) {
                inet_ntop(AF_INET, &sin->sin_addr, src_ip_out,
                          INET6_ADDRSTRLEN);
              }
            } else if (cmsg->cmsg_len >=
                           CMSG_LEN(sizeof(struct sockaddr_in6)) &&
                       is_ipv6) {
              struct sockaddr_in6 *sin6 =
                  (struct sockaddr_in6 *)(CMSG_DATA(cmsg) +
                                          sizeof(struct sock_extended_err));
              if (src_ip_out) {
                inet_ntop(AF_INET6, &sin6->sin6_addr, src_ip_out,
                          INET6_ADDRSTRLEN);
              }
            }
          }
...
// 接收 TCP 错误消息（从错误队列接收 ICMP Time Exceeded/Destination
// Unreachable）
static __attribute__((unused)) int
receive_tcp_error(socket_t tcp_sock, int timeout_ms, const char *target_ip,
                  int ttl, int is_ipv6, double *rtt_out, char *src_ip_out,
                  uint16_t src_port, uint64_t send_time) {
  if (tcp_sock < 0 || !target_ip || !rtt_out)
    return -1;
  ...
  ssize_t bytes_received =
      recvfrom(tcp_sock, data_buffer, sizeof(data_buffer), MSG_DONTWAIT,
               (struct sockaddr *)&from_addr, &from_len);
  if (bytes_received < 0) {
    // 尝试使用 recvmsg
    char control_buffer[256];
...
static int icmp_parallel_worker_start(icmp_parallel_worker_ctx *ctx,
                                      socket_t sock, mtr_event_queue *queue,
                                      int is_ipv6, int max_ttl, int times,
                                      uint16_t seq_xor_token) {
  if (!ctx || sock < 0 || !queue)
    return -1;
  ...
  int ret =
      pthread_create(&ctx->thread, NULL, icmp_parallel_worker_thread, ctx);
  if (ret != 0) {
    ctx->sock = -1;
    pthread_mutex_unlock(&ctx->mutex);
    return -1;
...
static int probe_icmp_parallel(const char *target_ip, int max_ttl, int times,
                               int timeout_ms, int is_ipv6,
                               socket_t icmp_socket,
                               cls_mtr_path_result *path) {
  if (!target_ip || icmp_socket < 0 || !path)
    return -1;
  ...
  session.session_start_time = get_monotonic_timestamp_ms();
  ...
  uint64_t token_seed = session.session_start_time ^ (uint64_t)getpid() ^
                        (uint64_t)(uintptr_t)&session;
  session.seq_xor_token = (uint16_t)(token_seed & 0xFFFF);
  if (session.seq_xor_token == 0)
    session.seq_xor_token = 0xA5A5;
  ...
  // 注意：最后一跳可能使用更慢的 MTR_TARGET_PROBE_GAP_US（防止
  // rate-limit），因此 deadline 估算要取更保守的 gap
  uint32_t worst_gap_us = MTR_PROBE_GAP_US;
  if (MTR_TARGET_PROBE_GAP_US > worst_gap_us)
    worst_gap_us = MTR_TARGET_PROBE_GAP_US;
  const uint64_t session_timeout_ms = mtr_compute_parallel_session_timeout_ms(
      max_ttl, times, timeout_ms, worst_gap_us);
  ...
  if (icmp_parallel_worker_start(&worker_ctx, icmp_socket, &event_queue,
                                 is_ipv6, max_ttl, times,
                                 session.seq_xor_token) != 0) {
  ...
  // 使用进程 ID 和时间戳生成非 0 的 ICMP ID，避免 ID=0
  // 可能导致某些包被过滤的问题 注意：在 iOS/macOS 上使用 SOCK_DGRAM
  // 时，内核可能会改写此 ID，但使用非 0 值作为基础更安全
  uint16_t icmp_identifier =
      (uint16_t)((getpid() ^ (uint16_t)(get_current_timestamp_ms() & 0xFFFF)) &
                 0xFFFF);
  ...
  while (1) {
    ...
    while (event_queue_try_pop(&event_queue, &event) == 0) {
      ...
      // 检查条件：probe_index 在有效范围内，且 send_times
      // 已设置（说明该探测已发送）
      if (event.probe_index >= 0 && event.probe_index < times &&
          hop->send_times[event.probe_index] > 0) {
        // 直接匹配指定的 probe_index
        if (hop->rtts[event.probe_index] == 0.0 &&
            hop->src_ips[event.probe_index][0] == '\0') {
        ...
        for (int i = 0; i < hop->sent_count && i < times; i++) {
          if (hop->rtts[i] == 0.0 && hop->src_ips[i][0] == '\0') {
            // 计算 RTT
            double calculated_rtt =
                (double)(event.recv_time - hop->send_times[i]);
            if (calculated_rtt > 0 &&
                calculated_rtt < 60000) { // 合理的 RTT 范围
              // 选择 RTT 最小的匹配（简单策略）
      ...
      if (match_idx >= 0 && event.src_ip[0] != '\0') {
        ...
        safe_strncpy(hop->src_ips[match_idx], event.src_ip,
                     sizeof(hop->src_ips[match_idx]));
        hop->recv_count++;
      }
    ...
    if (!session.target_reached) {
      // 遍历所有 TTL
      for (int ttl = 1; ttl <= max_ttl && !session.target_reached &&
                        !consecutive_timeout_stopped;
           ttl++) {
        mtr_hop_state *hop = &session.hops[ttl];
        ...
        if (ttl > 1 && hop->sent_count == 0) {
          if (mtr_check_consecutive_timeouts_and_stop(
                  session.hops, max_ttl, times, timeout_ms, current_time,
                  ttl - 1, path, NULL)) {
        ...
        if (hop->sent_count < times) {
          ...
          if (last_set_ttl != ttl) {
            ...
            int so_ret = 0;
            if (is_ipv6) {
              so_ret = setsockopt(icmp_socket, IPPROTO_IPV6, IPV6_UNICAST_HOPS,
                                  &ttl_value, sizeof(ttl_value));
            } else {
              so_ret = setsockopt(icmp_socket, IPPROTO_IP, IP_TTL, &ttl_value,
                                  sizeof(ttl_value));
            }
            if (so_ret < 0) {
              int e = errno;
              path->exceptionNum++;
              fatal_error_code =
                  (int)mtr_classify_syscall_failure("setsockopt", e);
          ...
          while (hop->sent_count < times) {
            ...
            if (send_icmp_echo(icmp_socket, target_ip, ttl, probe_idx,
                               icmp_identifier, session.seq_xor_token,
                               &send_time, &send_errno, &failed_op) == 0) {
              ...
              if (probes_since_event_check >= 3) {
                ...
                while (event_queue_try_pop(&event_queue, &quick_event) == 0 &&
                       event_processed < 10) {
                  ...
                  if (quick_ttl >= 1 && quick_ttl <= max_ttl) {
                    ...
                    // 检查条件：probe_index 在有效范围内，且 send_times
                    // 已设置（说明该探测已发送）
                    if (quick_event.probe_index >= 0 &&
                        quick_event.probe_index < times &&
                        quick_hop->send_times[quick_event.probe_index] > 0 &&
                        quick_event.src_ip[0] != '\0') {
                      if (quick_hop->rtts[quick_event.probe_index] == 0.0 &&
                          quick_hop->src_ips[quick_event.probe_index][0] ==
                              '\0') {
                        double quick_rtt =
                            (double)(quick_event.recv_time -
                                     quick_hop
                                         ->send_times[quick_event.probe_index]);
                        if (quick_rtt > 0 && quick_rtt < 60000) {
                          quick_hop->rtts[quick_event.probe_index] = quick_rtt;
                          safe_strncpy(
                              quick_hop->src_ips[quick_event.probe_index],
                              quick_event.src_ip,
                              sizeof(
                                  quick_hop->src_ips[quick_event.probe_index]));
                    ...
                    // 检查是否到达目标（确保 IP 不为空）
                    if (quick_event.src_ip[0] != '\0' &&
                        strcmp(quick_event.src_ip, target_ip) == 0) {
                      if (!session.target_reached) {
                        session.target_reached = 1;
              ...
              if (hop->sent_count < times) {
                uint32_t gap_us = current_ttl_target_reached
                                      ? MTR_TARGET_PROBE_GAP_US
                                      : MTR_PROBE_GAP_US;
                usleep(gap_us);
              }
            } else {
              ...
              path->exceptionNum++;
              hop->ttl_done = 1;
              // 对于明显的“不可达/无权限”错误，直接终止整个
              // session，避免无意义继续发送
              const char *op =
                  (failed_op && failed_op[0]) ? failed_op : "sendto";
              if (e != 0) {
              ...
              if (fatal_error_code ==
                      cls_mtr_detector_error_permission_denied ||
                  fatal_error_code ==
                      cls_mtr_detector_error_network_unreachable ||
                  fatal_error_code == cls_mtr_detector_error_host_unreachable ||
                  fatal_error_code ==
                      cls_mtr_detector_error_address_not_available ||
                  fatal_error_code ==
                      cls_mtr_detector_error_resource_exhausted) {
                for (int t = ttl; t <= max_ttl; t++) {
                  session.hops[t].ttl_done = 1;
                }
              }
          ...
          if (hop->sent_count >= times) {
            if (mtr_check_consecutive_timeouts_and_stop(
                    session.hops, max_ttl, times, timeout_ms, current_time, ttl,
                    path, NULL)) {
              consecutive_timeout_stopped = 1;
              break;
            }
    ...
    if (session.target_reached) {
      ...
      if (target_hop->first_send_time > 0) {
        ...
        // 检查目标 hop 是否完成：
        // 1. 已发送完本 hop
        // 的所有探测（sent_count>=times），且已接收完所有已发送的响应，或者
        // 2. 已发送完本 hop 的所有探测（sent_count>=times），且已超时
        int target_hop_completed = 0;
        if (target_hop->sent_count >= times &&
            target_hop->recv_count >= target_hop->sent_count) {
          target_hop_completed = 1;
        } else if (target_hop->sent_count >= times &&
    ...
    if (!session.target_reached) {
      if (mtr_check_consecutive_timeouts_and_stop(
              session.hops, max_ttl, times, timeout_ms, current_time, max_ttl,
              path, "probe_icmp_parallel")) {
        consecutive_timeout_stopped = 1;
      }
  ...
  // 未到达目标且触发提前停止（连续超时/全局 deadline），按失败返回（但 path
  // 仍包含已采样的部分结果）
...
static int probe_udp_parallel(const char *target_ip, int max_ttl, int times,
                              int timeout_ms, int is_ipv6, socket_t udp_socket,
                              socket_t icmp_socket, cls_mtr_path_result *path) {
  if (!target_ip || udp_socket < 0 || !path)
    return -1;
  ...
  if (base_max >= base_min) {
    ...
    if (range >= 256) {
      ...
      if (dst_port_base > base_max)
        dst_port_base = base_max;
    } else {
      dst_port_base = (uint16_t)(base_min + (uint16_t)(seed % range));
    }
  }

  if (udp_parallel_worker_start(&worker_ctx, udp_socket, icmp_socket,
                                &event_queue, is_ipv6, dst_port_base) != 0) {
  ...
  while (1) {
    ...
    while (udp_event_queue_try_pop(&event_queue, &event) == 0) {
      ...
      if (event.probe_index >= 0 && event.probe_index < times &&
        ...
        if (hop->rtts[event.probe_index] == 0.0 &&
            hop->src_ips[event.probe_index][0] == '\0') {
          double raw_rtt =
              (double)(event.recv_time - hop->send_times[event.probe_index]);
      ...
      // 检查是否到达目标（确保 IP 不为空）
      // - UDP：Port Unreachable / 源 IP 等于目标 IP 都视为到达目标
      if (event.src_ip[0] != '\0' &&
          (strcmp(event.src_ip, target_ip) == 0 || event.is_target_reply)) {
        if (!session.target_reached) {
          session.target_reached = 1;
          session.target_reached_ttl = ttl;
        }
      ...
      for (int ttl = 1; ttl <= max_ttl && !session.target_reached &&
        ...
        if (hop->sent_count < times) {
          if (last_set_ttl != ttl) {
            ...
            int so_ret = 0;
            if (is_ipv6) {
              so_ret = setsockopt(udp_socket, IPPROTO_IPV6, IPV6_UNICAST_HOPS,
                                  &ttl_value, sizeof(ttl_value));
            } else {
              so_ret = setsockopt(udp_socket, IPPROTO_IP, IP_TTL, &ttl_value,
                                  sizeof(ttl_value));
            }
            if (so_ret < 0) {
              int e = errno;
          ...
          while (hop->sent_count < times) {
            ...
            int send_errno = 0;
            const char *failed_op = NULL;
            if (send_udp_packet(udp_socket, target_ip, ttl, probe_idx,
                                dst_port_base, &src_port, &send_time,
                                &send_errno, &failed_op) == 0) {
              hop->send_times[probe_idx] = send_time;
              if (hop->sent_count == 0) {
                hop->first_send_time = send_time;
              }
              ...
              if (probes_since_event_check >= 3) {
                ...
                int event_processed = 0;
                while (udp_event_queue_try_pop(&event_queue, &quick_event) ==
                           0 &&
                       event_processed < 10) {
                  int quick_ttl = quick_event.ttl;
                  if (quick_ttl >= 1 && quick_ttl <= max_ttl) {
                    ...
                    if (quick_event.probe_index >= 0 &&
                      ...
                      if (quick_hop->rtts[quick_event.probe_index] == 0.0 &&
                          quick_hop->src_ips[quick_event.probe_index][0] ==
                              '\0') {
                        double raw_quick_rtt =
                            (double)(quick_event.recv_time -
                                     quick_hop
                                         ->send_times[quick_event.probe_index]);
                        quick_rtt = raw_quick_rtt >= 0.0 ? raw_quick_rtt
                                                         : -raw_quick_rtt;
                      ...
                      if (quick_hop->sent_count > 0) {
                        for (int i = 0; i < quick_hop->sent_count && i < times;
                             i++) {
                          if (quick_hop->send_times[i] > 0 &&
                              quick_hop->rtts[i] == 0.0 &&
                              quick_hop->src_ips[i][0] == '\0') {
                            double raw_rtt = (double)(quick_event.recv_time -
                                                      quick_hop->send_times[i]);
                            double abs_rtt =
                                raw_rtt >= 0.0 ? raw_rtt : -raw_rtt;
                            if (abs_rtt > 0.0 && abs_rtt < 60000.0) {
                              if (quick_match_idx == -1 ||
                                  abs_rtt < quick_rtt) {
                                quick_match_idx = i;
                                quick_rtt = abs_rtt;
                              }
                    ...
                    // UDP：到达目标的快速判断（Port Unreachable / 源 IP
                    // 等于目标 IP）
                    if (strcmp(quick_event.src_ip, target_ip) == 0 ||
                        quick_event.is_target_reply) {
                      if (!session.target_reached) {
                        session.target_reached = 1;
                        session.target_reached_ttl = quick_ttl;
                        if (quick_ttl == ttl) {
    ...
    if (session.target_reached) {
      ...
      if (target_hop->sent_count >= times &&
          (target_hop->recv_count >= times ||
           target_elapsed > (uint64_t)timeout_ms)) {
        break;
      }
    } else {
      if (mtr_check_consecutive_timeouts_and_stop(
              session.hops, max_ttl, times, timeout_ms, current_time, max_ttl,
              path, "probe_udp_parallel")) {
        consecutive_timeout_stopped = 1;
        break;
      }
...
static int probe_tcp_parallel(const char *target_ip, int max_ttl, int times,
                              int timeout_ms, int is_ipv6, uint16_t tcp_dst_port,
                              socket_t *tcp_sockets, int tcp_socket_count,
                              socket_t icmp_sock, unsigned int interface_index,
                              cls_mtr_path_result *path) {
  if (!target_ip || !tcp_sockets || tcp_socket_count <= 0 || !path)
    return -1;
  ...
  session.times = times;
  session.timeout_ms = timeout_ms;
  session.tcp_dst_port = tcp_dst_port;
  ...
  // 注意：最后一跳可能使用更慢的 MTR_TARGET_PROBE_GAP_US（防止
  // rate-limit），因此 deadline 估算要取更保守的 gap 另外：send_tcp_syn
  // 内部还有固定等待（确保 SYN 发出），也需要计入发送阶段上界，否则可能提前
  // deadline 导致最后一跳 responseNum 偏小
  uint32_t worst_gap_us = MTR_TCP_PROBE_GAP_US;
  if (MTR_TARGET_PROBE_GAP_US > worst_gap_us)
    worst_gap_us = MTR_TARGET_PROBE_GAP_US;
  uint32_t worst_per_probe_us = worst_gap_us + MTR_TCP_SEND_OVERHEAD_US;
  const uint64_t session_timeout_ms = mtr_compute_parallel_session_timeout_ms(
      max_ttl, times, timeout_ms, worst_per_probe_us);
  const uint64_t deadline_ms = session.session_start_time + session_timeout_ms;
  ...
  // 为本次 TCP MTR session
  // 选择一个源端口基址（避免与系统临时端口冲突，并保证空间足够） src_port =
  // base + (ttl<<8) + probe_index
  ...
  uint16_t src_port_base =
      (uint16_t)(base_min +
                 (uint16_t)((seed % ((base_max - base_min + 1) / 256 + 1)) *
                            256));
  if (src_port_base > base_max)
    src_port_base = base_max;
  ...
  if (tcp_parallel_worker_start(&worker_ctx, tcp_sockets, tcp_socket_count,
                                icmp_sock, &event_queue, is_ipv6, target_ip,
                                src_port_base, tcp_dst_port) != 0) {
  ...
  while (1) {
    ...
    while (tcp_event_queue_try_pop(&event_queue, &event) == 0) {
      ...
      // TCP“到达目标”判定（更保守）：
      // - connect 成功（event.is_target_reply=1）视为强证据
      // -
      // ECONNREFUSED（event.is_target_refused=1）视为弱证据：需要“多次一致性确认
      // + 与前一跳有回包交叉验证”
      if (event.is_target_reply) {
        hop->tcp_connected_count++;
        if (!session.target_reached) {
          session.target_reached = 1;
          session.target_reached_ttl = ttl;
        }
      ...
      } else if (event.is_target_refused) {
        hop->tcp_refused_count++;
        int prev_ok = (ttl <= 1) ? 1 : (session.hops[ttl - 1].recv_count > 0);
        if (!session.target_reached && hop->tcp_refused_count >= 2 && prev_ok) {
          session.target_reached = 1;
          session.target_reached_ttl = ttl;
          hop->ttl_done = 1;
          for (int t = ttl + 1; t <= max_ttl; t++) {
      ...
      for (int ttl = 1; ttl <= max_ttl && !session.target_reached &&
        ...
        if (hop->sent_count < times) {
          ...
          while (hop->sent_count < times) {
            ...
            while (slot < 0) {
              ...
              for (int si = 0; si < tcp_socket_count; si++) {
                ...
                if (is_ipv6) {
                  struct sockaddr_in6 la6;
                  socklen_t llen = sizeof(la6);
                  if (getsockname(s, (struct sockaddr *)&la6, &llen) == 0)
                    lp = ntohs(la6.sin6_port);
                } else {
                  struct sockaddr_in la4;
                  socklen_t llen = sizeof(la4);
                  if (getsockname(s, (struct sockaddr *)&la4, &llen) == 0)
                    lp = ntohs(la4.sin_port);
                }
                int dt = 0, dp = -1;
                if (lp != 0 && tcp_probe_decode_from_src_port(lp, src_port_base,
                                                              &dt, &dp) == 0) {
                  if (dt >= 1 && dt <= max_ttl && dp >= 0 && dp < times) {
                    mtr_hop_state *hh = &session.hops[dt];
                    uint64_t st = hh->send_times[dp];
                    if (st > 0 && hh->rtts[dp] == 0.0 &&
                        hh->src_ips[dp][0] == '\0') {
              ...
              usleep(10000);
              current_time = get_monotonic_timestamp_ms();
            }

            socket_t tcp_sock =
                create_tcp_probe_socket(is_ipv6, interface_index, bind_port);
            if (tcp_sock < 0) {
              path->bindFailed++;
              path->exceptionNum++;
              hop->ttl_done = 1;
              // 端口绑定/资源耗尽/权限等属于“可解释”的致命错误，直接停止
              // session
              int e = errno;
              if (e != 0) {
            ...
            const char *failed_op = NULL;
            int send_result =
                send_tcp_syn(tcp_sock, target_ip, ttl, probe_idx,
                             session.tcp_dst_port, NULL, &send_time, &send_errno,
                             &failed_op);
            if (send_result == 0) {
              hop->send_times[probe_idx] = send_time;
              if (hop->sent_count == 0) {
              ...
              if (probes_since_event_check >= 3) {
                ...
                int event_processed = 0;
                while (tcp_event_queue_try_pop(&event_queue, &quick_event) ==
                           0 &&
                       event_processed < 10) {
                  int quick_ttl = quick_event.ttl;
                  if (quick_ttl >= 1 && quick_ttl <= max_ttl) {
                    ...
                    // 快速判断“到达目标”（TCP 更保守）
                    if (quick_event.is_target_reply) {
                      quick_hop->tcp_connected_count++;
                      if (!session.target_reached) {
                        session.target_reached = 1;
                        session.target_reached_ttl = quick_ttl;
                        if (quick_ttl == ttl) {
                    ...
                    } else if (quick_event.is_target_refused) {
                      quick_hop->tcp_refused_count++;
                      int prev_ok =
                          (quick_ttl <= 1)
                              ? 1
                              : (session.hops[quick_ttl - 1].recv_count > 0);
                      if (!session.target_reached &&
                          quick_hop->tcp_refused_count >= 2 && prev_ok) {
                        session.target_reached = 1;
                        session.target_reached_ttl = quick_ttl;
                        if (quick_ttl == ttl) {
                          current_ttl_target_reached = 1;
              ...
              if (e != 0) {
                const char *op =
                    (failed_op && failed_op[0]) ? failed_op : "connect";
                fatal_error_code = (int)mtr_classify_syscall_failure(op, e);
                mtr_record_fatal_error(path, op, e);
                if (fatal_error_code ==
                        cls_mtr_detector_error_permission_denied ||
                    fatal_error_code ==
                        cls_mtr_detector_error_network_unreachable ||
                    fatal_error_code ==
                        cls_mtr_detector_error_host_unreachable ||
                    fatal_error_code ==
                        cls_mtr_detector_error_address_not_available ||
...
static socket_t create_socket(const char *protocol, int is_ipv6,
  ...
  if (interface_index > 0) {
    if (is_ipv6) {
      ...
      if (setsockopt(sock, IPPROTO_IPV6, IPV6_BOUND_IF, &interface_index,
      ...
      if (setsockopt(sock, IPPROTO_IP, IP_BOUND_IF, &interface_index,
                     sizeof(interface_index)) < 0) {
        if (out_bind_failed)
          *out_bind_failed = 1;
        if (out_errno)
...
static void resolve_local_ip_for_target(const char *resolved_ip, int is_ipv6,
                                        char *out_ip, size_t out_len) {
  if (!resolved_ip || !out_ip || out_len == 0)
    return;
...
static int mtr_get_ifname_for_local_ip(const char *local_ip, int is_ipv6,
                                       char *out_ifname, size_t out_len) {
  ...
  if (inet_pton(family, local_ip,
                is_ipv6 ? (void *)&target_v6 : (void *)&target_v4) != 1) {
...
// 当未显式指定 interface_index 时，选择一个“看起来最可能”的活动接口名（用于上报
// interface_name） 优先级：en0（WiFi）> 任意 pdp_ip*（蜂窝）>
// en1（以太网/其他）> 任意 UP/RUNNING 非 loopback
static int mtr_pick_active_ifname(int is_ipv6, char *out_ifname,
                                  size_t out_len) {
  if (!out_ifname || out_len == 0)
    return -1;
  out_ifname[0] = '\0';
...
// 执行 MTR 探测的核心逻辑
cls_mtr_detector_error_code
cls_mtr_detector_perform_mtr(const char *target,
                             const cls_mtr_detector_config *_Nullable config,
                             cls_mtr_detector_result *result) {
  if (!result)
    return cls_mtr_detector_error_invalid_param;
  ...
  // 解析配置
  const char *protocol =
      (config && config->protocol) ? config->protocol : "icmp";
  int max_ttl =
      (config && config->max_ttl > 0) ? config->max_ttl : MTR_DEFAULT_MAX_TTL;
  int timeout_ms = (config && config->timeout_ms > 0) ? config->timeout_ms
                                                      : MTR_DEFAULT_TIMEOUT_MS;
  ...
  int tcp_dst_port =
      (config && config->tcp_dst_port > 0) ? config->tcp_dst_port : MTR_TCP_DST_PORT;
  ...
  if (times > 30)
    times = 30;
  if (tcp_dst_port < 1)
    tcp_dst_port = MTR_TCP_DST_PORT;
  if (tcp_dst_port > 65535)
    tcp_dst_port = 65535;
  ...
  if (resolve_hostname(target, prefer, resolved_ip) != 0) {
    ...
    safe_strncpy(result->error_message, "Failed to resolve hostname",
  ...
  resolve_local_ip_for_target(resolved_ip, is_ipv6, device_ip,
                              sizeof(device_ip));
  ...
  if (!result->paths) {
    ...
    safe_strncpy(result->error_message, "Failed to allocate result paths",
                 sizeof(result->error_message));
    return final_code;
  }
  ...
  // - 若未指定（interface_index==0）：优先用“源 IP ->
  // ifaddrs”反查，失败则按活动接口优先级兜底
  {
    ...
    if (interface_index > 0) {
      if (if_indextoname(interface_index, ifname) == NULL ||
        ...
        snprintf(path->interface_name, sizeof(path->interface_name),
                 "ifindex:%u", interface_index);
      }
    } else {
      // 默认网卡：尽量给出真实接口名（en0/pdp_ip0/...），便于上层判断 WiFi/蜂窝
      if (device_ip[0] != '\0') {
        (void)mtr_get_ifname_for_local_ip(device_ip, is_ipv6, ifname,
                                          sizeof(ifname));
      }
      if (ifname[0] == '\0') {
  ...
  if (strcmp(protocol, "icmp") == 0) {
    int bind_failed = 0;
    int sock_errno = 0;
    icmp_socket = create_socket("icmp", is_ipv6, interface_index, &bind_failed,
                                &sock_errno);
    if (icmp_socket < 0) {
      if (bind_failed) {
      ...
      snprintf(result->error_message, sizeof(result->error_message),
               "%s (errno=%d:%s)",
               bind_failed ? "Failed to bind ICMP socket to interface"
    ...
    int bind_failed = 0;
    int sock_errno = 0;
    udp_socket = create_socket("udp", is_ipv6, interface_index, &bind_failed,
                               &sock_errno);
    if (udp_socket < 0) {
      if (bind_failed) {
        final_code = cls_mtr_detector_error_net_binding_failed;
      ...
      snprintf(result->error_message, sizeof(result->error_message),
               "%s (errno=%d:%s)",
               bind_failed ? "Failed to bind UDP socket to interface"
    ...
    if (icmp_socket < 0) {
      ...
      snprintf(result->error_message, sizeof(result->error_message),
               "%s (errno=%d:%s)",
               bind_failed
                   ? "Failed to bind ICMP recv socket for UDP to interface"
                   : "Failed to create ICMP recv socket for UDP",
               sock_errno, (sock_errno > 0 ? strerror(sock_errno) : "unknown"));
      if (bind_failed)
        path->bindFailed++;
      goto cleanup;
    ...
    if (!tcp_sockets) {
      final_code = cls_mtr_detector_error_socket_create_error;
      result->error_code = final_code;
      safe_strncpy(result->error_message, "Failed to allocate TCP socket array",
                   sizeof(result->error_message));
      goto cleanup;
    }

    // TCP 并行探测采用“每个 probe 新建 socket 并绑定源端口编码
    // TTL/probe_index”的策略； 这里的数组作为并发槽位，初始化为 -1，具体 socket
    // 在 probe_tcp_parallel 内按需创建/释放。
    for (int i = 0; i < socket_count; i++) {
      tcp_sockets[i] = -1;
    }
    tcp_socket_count = socket_count;

    // 额外创建 ICMP socket 用于接收 ICMP 错误消息（Time Exceeded、Destination
    // Unreachable 等）
    int bind_failed = 0;
    int sock_errno = 0;
    icmp_socket = create_socket("icmp", is_ipv6, interface_index, &bind_failed,
    ...
    if (icmp_socket < 0) {
      ...
      snprintf(result->error_message, sizeof(result->error_message),
               "%s (errno=%d:%s)",
               bind_failed
                   ? "Failed to bind ICMP recv socket for TCP to interface"
                   : "Failed to create ICMP recv socket for TCP",
               sock_errno, (sock_errno > 0 ? strerror(sock_errno) : "unknown"));
      if (bind_failed)
        path->bindFailed++;
      goto cleanup;
    ...
    safe_strncpy(result->error_message,
                 "Unsupported protocol, only icmp/udp/tcp are supported",
                 sizeof(result->error_message));
    goto cleanup;
  }
  ...
  if (strcmp(protocol, "icmp") == 0) {
    // ICMP 使用并行探测
    int probe_result = probe_icmp_parallel(
        resolved_ip, max_ttl, times, timeout_ms, is_ipv6, icmp_socket, path);
    if (probe_result == 0) {
      last_hop = path->lastHop;
      if (last_hop <= 0) {
        // 没有任何 hop 响应：按超时返回（更明确）
        final_code = (path->exceptionNum > 0)
                         ? cls_mtr_detector_error_socket_create_error
      ...
      result->error_code = final_code;
      safe_strncpy(result->error_message,
                   "Probe timeout: target host not reached",
                   sizeof(result->error_message));
      ...
      if (path->last_errno != 0 && path->last_error_op[0] != '\0') {
        snprintf(result->error_message, sizeof(result->error_message),
                 ...
                 path->last_error_op, path->last_errno,
                 strerror(path->last_errno));
      } else {
        snprintf(result->error_message, sizeof(result->error_message),
                 "Probe failed: %s (code=%ld)", mtr_error_name(final_code),
                 (long)final_code);
      }
    } else {
      final_code = (path->exceptionNum > 0)
      ...
      result->error_code = final_code;
      safe_strncpy(result->error_message,
                   (path->exceptionNum > 0) ? "Probe failed: socket/send error"
                                            : "Probe failed: unknown error",
                   sizeof(result->error_message));
    }
  } else if (strcmp(protocol, "udp") == 0) {
    // UDP 使用并行探测
    int probe_result =
        probe_udp_parallel(resolved_ip, max_ttl, times, timeout_ms, is_ipv6,
                           udp_socket, icmp_socket, path);
    if (probe_result == 0) {
      last_hop = path->lastHop;
      if (last_hop <= 0) {
        final_code = (path->exceptionNum > 0)
    ...
    // TCP 使用并行探测
    int probe_result = probe_tcp_parallel(
        resolved_ip, max_ttl, times, timeout_ms, is_ipv6, (uint16_t)tcp_dst_port,
        tcp_sockets, (int)tcp_socket_count, icmp_socket, interface_index, path);
    if (probe_result == 0) {
      last_hop = path->lastHop;
      if (last_hop <= 0) {
        final_code = (path->bindFailed > 0)
                         ? cls_mtr_detector_error_net_binding_failed
                         : ((path->exceptionNum > 0)
                                ? cls_mtr_detector_error_socket_create_error
                                : cls_mtr_detector_error_timeout);
        result->error_code = final_code;
        safe_strncpy(result->error_message,
                     (path->exceptionNum > 0 || path->bindFailed > 0)
...
// 将 MTR 结果转换为 JSON 格式
static int json_append(char *buf, size_t buf_size, size_t *used,
                       const char *fmt, ...) {
  if (!buf || buf_size == 0 || !used || !fmt)
    return -1;
  if (*used >= buf_size)
    return -1;
...
static int json_append_escaped_string(char *buf, size_t buf_size, size_t *used,
                                      const char *s) {
  if (!s)
    s = "";
  if (json_append(buf, buf_size, used, "\"") != 0)
    return -1;
  ...
  for (const unsigned char *p = (const unsigned char *)s; *p; p++) {
    ...
    case '\\':
      ...
      break;
    case '\b':
      if (json_append(buf, buf_size, used, "\\b") != 0)
        return -1;
      break;
    case '\f':
      if (json_append(buf, buf_size, used, "\\f") != 0)
        return -1;
      break;
    case '\n':
      ...
      break;
    case '\r':
      if (json_append(buf, buf_size, used, "\\r") != 0)
        return -1;
      break;
    case '\t':
      if (json_append(buf, buf_size, used, "\\t") != 0)
    ...
    default:
      if (c < 0x20) {
        if (json_append(buf, buf_size, used, "\\u%04x", (unsigned int)c) != 0)
          return -1;
      } else {
        if (json_append(buf, buf_size, used, "%c", (char)c) != 0)
          return -1;
      }
      break;
    }
...
int cls_mtr_detector_result_to_json(const cls_mtr_detector_result *result,
  ...
  // 缓冲区安全检查说明：
  // 本函数通过 json_append 辅助函数进行所有字符串写入操作。
  // json_append 会在每次写入前检查剩余缓冲区空间，如果空间不足会返回 -1。
  // 本函数在每次 json_append 调用后立即检查返回值，一旦检测到缓冲区不足（返回 -1），
  // 会立即返回 -1，不会继续写入，从而避免缓冲区溢出。
  // 注意：当缓冲区不足时，已写入的数据可能不完整，调用者不应使用部分写入的数据。

  // 从 result 结构体中获取错误码，确保数据一致性
  cls_mtr_detector_error_code error_code = result->error_code;

  size_t used = 0;
  if (json_append(json_buffer, buffer_size, &used, "{") != 0)
    return -1;

  if (json_append(json_buffer, buffer_size, &used, "\"method\":") != 0)
    return -1;
  if (json_append_escaped_string(json_buffer, buffer_size, &used,
                                 result->method) != 0)
    return -1;
  if (json_append(json_buffer, buffer_size, &used, ",\"host\":") != 0)
    return -1;
  if (json_append_escaped_string(json_buffer, buffer_size, &used,
                                 result->target) != 0)
    return -1;
  ...
  // 首层 type 使用探测协议类型
  const char *top_type =
      (result->paths_count > 0 && result->paths && result->paths[0].protocol[0])
          ? result->paths[0].protocol
          : "mtr";
  if (json_append(json_buffer, buffer_size, &used, ",\"type\":") != 0)
    return -1;
  if (json_append_escaped_string(json_buffer, buffer_size, &used, top_type) !=
      0)
    return -1;

  if (json_append(json_buffer, buffer_size, &used, ",\"max_paths\":%d",
                  result->max_paths > 0 ? result->max_paths : 1) != 0)
    return -1;

  // paths 数组
  if (json_append(json_buffer, buffer_size, &used, ",\"paths\":[") != 0)
    return -1;

  const size_t paths_count =
      (result->paths && result->paths_count > 0) ? result->paths_count : 0;
  for (size_t i = 0; i < paths_count; i++) {
    ...
    if (json_append(json_buffer, buffer_size, &used, ",\"host_ip\":") != 0)
      return -1;
    if (json_append_escaped_string(json_buffer, buffer_size, &used,
                                   path->host_ip[0] ? path->host_ip : "") != 0)
      return -1;

    if (json_append(json_buffer, buffer_size, &used, ",\"type\":\"path\"") != 0)
      return -1;

    // 直接使用已构造的 path（timestamp:源IP-目标IP）
    if (json_append(json_buffer, buffer_size, &used, ",\"path\":") != 0)
      return -1;
    if (json_append_escaped_string(json_buffer, buffer_size, &used,
                                   path->path) != 0)
      return -1;

    if (json_append(json_buffer, buffer_size, &used, ",\"lastHop\":%d",
                    path->lastHop) != 0)
      return -1;
    if (json_append(json_buffer, buffer_size, &used, ",\"timestamp\":%lld",
                    path->timestamp) != 0)
      return -1;

    // interface_name：紧跟 timestamp / protocol 输出，方便上层直接消费
    if (json_append(json_buffer, buffer_size, &used, ",\"interface\":") !=
        0)
      return -1;
    if (json_append_escaped_string(
            json_buffer, buffer_size, &used,
            path->interface_name[0] ? path->interface_name : "") != 0)
      return -1;

    if (json_append(json_buffer, buffer_size, &used, ",\"protocol\":") != 0)
      return -1;
    if (json_append_escaped_string(json_buffer, buffer_size, &used,
                                   path->protocol) != 0)
      return -1;

    if (json_append(json_buffer, buffer_size, &used, ",\"exceptionNum\":%d",
                    path->exceptionNum) != 0)
      return -1;
    if (json_append(json_buffer, buffer_size, &used, ",\"bindFailed\":%d",
                    path->bindFailed) != 0)
      return -1;

    // result 数组
    if (json_append(json_buffer, buffer_size, &used, ",\"result\":[") != 0)
      return -1;
    ...
    if (path->results && path->results_count > 0) {
      for (size_t j = 0; j < path->results_count; j++) {
        ...
        if (h->responseNum > 0) {
          path_has_any_reply = 1;
          break;
        }
        if (h->ip[0] != '\0' && strcmp(h->ip, "*") != 0) {
          path_has_any_reply = 1;
          break;
        }
      }
    ...
    if (path_has_any_reply && path->results && path->results_count > 0) {
      output_count =
          (path->lastHop > 0) ? (size_t)path->lastHop : path->results_count;
      if (output_count > path->results_count)
        output_count = path->results_count;
    }
    ...
    if (!path_has_any_reply) {
      ...
      if (json_append(json_buffer, buffer_size, &used, ",\"responseNum\":%d",
                      0) != 0)
        return -1;
      if (json_append(json_buffer, buffer_size, &used, ",\"ip\":") != 0)
        return -1;
      if (json_append_escaped_string(json_buffer, buffer_size, &used, "*") != 0)
        return -1;
  ...
  // 可选错误信息
  if (error_code != cls_mtr_detector_error_success ||
      (result->error_message[0] != '\0')) {
    if (json_append(json_buffer, buffer_size, &used, ",\"errCode\":%ld",
                    (long)error_code) != 0)
      return -1;
    if (result->error_message[0] != '\0') {
      if (json_append(json_buffer, buffer_size, &used, ",\"errMsg\":") != 0)
        return -1;
      if (json_append_escaped_string(json_buffer, buffer_size, &used,
